...
export class IPLookupController {
  ...
  async getLookupIP(request: FastifyRequest, reply: FastifyReply): Promise<IResOutPut> {
  ...
  async deleteByIP(request: FastifyRequest, reply: FastifyReply): Promise<IResOutPut> {
    const { ip } = request.params as { ip: string };
